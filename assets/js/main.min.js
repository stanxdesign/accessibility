    ...
    sections.forEach(section => {
      ...
      section.addEventListener("click", () => {
        ...
        sections.forEach(sec => {
          ...
          // 還原陰影樣式
          if (sec.classList.contains("shadow-dark")) {
            sec.classList.remove("shadow-dark");
          }
        });
        ...
        // 添加展開的陰影樣式
        section.classList.add("shadow-dark");
      });
    });
  });
});
...
// 處理物件隱藏與顯示的無障礙標籤
function objShowed(inputs, label){
	inputs.forEach(ta => {
		ta.disabled = false; // 啟用
		ta.setAttribute('tabindex', '0'); // 恢復 Tab 導覽
		ta.setAttribute('aria-disabled', 'false'); // 更新無障礙標示
	});
}

function objHidden(inputs, label){
	inputs.forEach(ta => {
		ta.disabled = true; // 停用
		ta.value = ''; // 清除內容
