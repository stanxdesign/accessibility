...
const moreOptioninputs = document.querySelectorAll('#moreTags .tag', '.');
const tagBtns = document.querySelector('#moreTags');
const mapLayer = document.querySelector('#map');

function objShowed(elements, className) {
  if (!elements) return;
...
function objHidden(elements, className) {
  if (!elements) return;

  // 如果是單一元素而非集合，將其轉換為陣列以便統一處理
  const elementsArray = elements.length ? elements : [elements];
...
function wcagAttribute() {
	const commonTags = document.querySelectorAll('#filterPanelForm .tag');
